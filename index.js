...
module.exports = function PacketsLogger(mod) {
  ...
  command.add("logC", () => {
    logC = !logC;
    command.message(
      `Client packet logging is now ${logC ? "enabled" : "disabled"}.`
    );
  ...
  command.add("logS", () => {
    logS = !logS;
    command.message(
      `Server packet logging is now ${logS ? "enabled" : "disabled"}.`
    );
    if (!logC && !logS && hookEnabled) disableHook();
    if ((logC || logS) && !hookEnabled) enableHook();
  });
  ...
  command.add("logRaw", (arg) => {
    ...
    command.message(
      `Raw packet logging is now ${logRaw ? "enabled" : "disabled"}${
        logRawUnkOnly ? " (only unknown packets)" : ""
      }.`
    );
  });
  ...
  command.add("logJson", () => {
    logJson = !logJson;
    command.message(
      `Json packet logging is now ${logJson ? "enabled" : "disabled"}.`
    );
  });
  ...
  command.add("logPaste", () => {
    logPaste = !logPaste;
    command.message(
      `Raw packet pasting format is now ${logPaste ? "enabled" : "disabled"}.`
    );
  });
  ...
  command.add("logUnk", (arg) => {
    ...
    command.message(
      `Unknown packet logging is now ${logUnk ? "enabled" : "disabled"}${
        logUnkOnly ? " (only)" : ""
      }.`
    );
  });
  ...
  //
  let playerName = null;
  let playerHooked1 = true;
  let playerId = null;
  command.add("logT", (arg) => {
    if (arg != null) {
      command.message(`Target packet logging is now ${arg}.`);
      playerName = arg;
      playerHooked1 = false;
    }
  });
  ...
  function internalType(data) {
    return (
      (data.$fake ? "[CRAFTED]	" : "") +
      (data.$silenced ? "[BLOCKED]	" : "") +
      (data.$modified ? "[EDITED]	" : "") +
      (!data.$fake && !data.$silenced && !data.$modified ? "         	" : "")
    );
  }

  function printUnknown(code, data, incoming, name) {
    logFile.write(
      `${timestamp()} ${packetArrow(incoming)} ${internalType(
        data
      )}    (id ${code}) ${name}\n`
    );
    if (logRaw) {
  ...
  function printKnown(name, packet, incoming, code, data, defPerhapsWrong) {
    ...
    logFile.write(
      `${timestamp()} ${packetArrow(incoming)} ${internalType(
        data
      )} ${name}    (id ${code}${defPerhapsWrong ? ", DEF WRONG!!!)" : ")"}\n`
    );
  ...
  function enableHook() {
    ...
    hook = mod.hook(
      ...
      (code, data, incoming, fake) => {
        if (!logC && !logS) return;
        if (playerId != null && data.gameId != playerId) return;
        //let protocolVersion = mod.protocolVersion
        let name = null;
        let packet = null;
        ...
        if (name) {
          ...
          if (packet) {
            let defPerhapsWrong = isDefPerhapsWrong(
              name,
              packet,
              incoming,
              data
